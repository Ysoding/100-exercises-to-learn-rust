...
impl Ticket {
    pub fn new(title: String, description: String, status: Status) -> Result<Ticket, String> {
        if title.is_empty() {
            return Err(format!("Title cannot be empty"));
        }
        if title.len() > 50 {
            return Err(format!("Title cannot be longer than 50 bytes"));
        }
        if description.is_empty() {
            return Err(format!("Description cannot be empty"));
        }
        if description.len() > 500 {
            return Err(format!("Description cannot be longer than 500 bytes"));
        }
