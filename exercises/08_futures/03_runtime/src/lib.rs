    ...
    T: Display + Send + Sync + 'static,
{
    ...
    let handle1 = tokio::spawn(work(first, reply.clone()));
    let handle2 = tokio::spawn(work(second, reply));

    tokio::try_join!(handle1, handle2).unwrap();
}

pub async fn work<T>(listener: TcpListener, reply: Arc<T>)
where
    T: Display + Send + Sync + 'static,
{
    loop {
        match listener.accept().await {
            Ok((mut socket, _)) => {
                ...
                tokio::spawn(async move { socket.write_all(reply.to_string().as_bytes()).await });
            }
            Err(e) => {
